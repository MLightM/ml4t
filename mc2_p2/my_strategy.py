...
def bollinger_strategy():
    ...
    # rm_IBM_shorter = pd.rolling_mean(df_IBM['IBM'], window=8)
    ...
    for date in df_IBM.index[20:]:
        ...
        # if rm_IBM_shorter[date] < rm_IBM_shorter[yestoday]:
        #     IBM_rm_shorter_delta += 1
        # else:
        #     IBM_rm_shorter_delta = 0
    ...
    daily_portvals = daily_portvals['2008-02-28':'2009-12-29'].rename(columns={"Daily_portval":"Portfolio"})
    # print type(daily_portvals)
    norm_portvals = daily_portvals/daily_portvals.ix[0]
    norm_portvals.plot(color='b')
    SPX_portvals = df_IBM['$SPX']['2008-02-28':'2009-12-29']
    SPX_norm = SPX_portvals/SPX_portvals.ix[0]
    SPX_norm.plot(color='g',label='$SPX')
    plt.show()
